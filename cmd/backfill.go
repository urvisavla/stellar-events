...
// =============================================================================
// Backfill Command — bulk historical ingestion writing cold segments directly
// =============================================================================
...
func runBackfill(cfg *config.Config, args []string) {
	...
	fs.Usage = func() {
		fmt.Fprintf(os.Stderr, "Usage: backfill [options]\n\n")
		fmt.Fprintf(os.Stderr, "Bulk-ingests historical events directly to cold segment files.\n\n")
	...
	cmdBackfill(cfg, uint32(*start), uint32(*end))
}
...
func cmdBackfill(cfg *config.Config, startLedger, endLedger uint32) {
	...
	// Backfill writes directly to cold/ subdirectory.
	// Override the segment path so Store writes to <segmentPath>/cold/.
	if cfg.Storage.SegmentPath != "" {
		cfg.Storage.SegmentPath = filepath.Join(cfg.Storage.SegmentPath, "cold")
	}
	...
	fmt.Fprintf(os.Stderr, "Backfilling events from ledgers %d to %d...\n", startLedger, endLedger)
	...
	summary.WriteString("\n")
	summary.WriteString("=== Backfill Complete ===\n")
	summary.WriteString("\n")
